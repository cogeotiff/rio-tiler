...
def test_get_array_statistics():
    ...
    # test with fully masked data
    arr = np.ma.array(np.zeros((1, 256, 256)))
    arr.mask = True
    stats = utils.get_array_statistics(arr)
    assert math.isnan(stats[0]["min"])
    assert math.isnan(stats[0]["max"])
    assert math.isnan(stats[0]["mean"])
    assert math.isnan(stats[0]["percentile_2"])
    assert math.isnan(stats[0]["percentile_98"])
    assert stats[0]["valid_pixels"] == 0
    assert stats[0]["valid_percent"] == 0
    assert stats[0]["masked_pixels"] == 65536
